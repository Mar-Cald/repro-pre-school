...
::: notes
It means your workflow is transparent. Every number in your paper, every figure in your presentation, has a clear lineage. You can trace it back to the raw data and see how it was generated.
:::
...
::: nonincremental
-   **Data**: How to organize, document, and share your datasets in ways that are usable by others and understandable by you—even years later.
-   **Code**: How to write analysis scripts that are clean, transparent, and reusable..
-   **Literate Programming**: How to combine code and text in the same document using tools like Quarto, so your reports, papers, and presentations are dynamic and replicable.
-   **Version Control and Sharing**: How to track changes, collaborate, and make your work openly available using tools like GitHub and OSF.\
:::
...
::: notes
So how do we do this? Let’s start with some concrete recommendations. Use plain-text formats for your data: CSV (.csv) and tab-delimited text (.txt) are ideal. They’re easy to read, easy to import, and future-proof. Avoid Excel files if you can—especially those with multiple sheets, embedded charts, or merged cells. Binary formats like .sav (SPSS), .dta (Stata), or .xlsx often create compatibility issues. If you do need to share those formats, also provide a CSV version. Plain text is portable. It doesn’t care what operating system or software someone uses. That’s what you want.
:::
...
A **README** file is the first thing someone sees when they open your dataset folder. It should answer basic questions like: - What is this dataset? - Who created it? - How was it collected? - What are the main variables? - How should I cite it?
...
Scripts are: - Reproducible: You can rerun them. - Documented: You can see what you did and when. - Shareable: Others can inspect and reproduce your analysis.
...
::: notes
If you’ve ever found yourself writing the same line of code over and over again, copying and pasting blocks of logic for different variables, or modifying dozens of lines when one thing changes—this section is for you. Functional programming is not some abstract theory. It’s a way to write clearer, shorter, and less error-prone code. And it’s a huge asset for reproducible science.
:::
...
::: notes
So far, we’ve talked about good data practices, clean code, and functional programming. But now we need to ask: Where does all of this live? How do we keep everything together—our data, our scripts, our figures, our notes—in a way that is coherent, portable, and reproducible? That’s what this section is about: project organization. Just like a clean kitchen makes it easier to cook, a clean research folder makes it easier to do science. And this is where R Projects, proper file structure, and tools like rrtools and renv come into play.
:::
...
**Absolute** path: read.csv("Users/tita/workinMemo/data/clean_data.csv") <br/>

**Relative** path: read.csv("data/clean_data.csv") <br/>
...
rrtools::create_compendium("my-awesome-project")
```
...
You write some code today using `dplyr 1.1.2`. In six months, `dplyr` gets updated... :cry:
...
`renv::snapshot()` \# save current state <br/>

`renv::update()` \# update lockfile <br/>
...
-   If you change a plot’s colors, the new version appears—without having to re-export and re-insert anything.
:::
...
## Commit Message Tips :writing_hand:
...
I won’t lie: Git can be confusing at first. The terminology—commits, branches, merges, remotes—can feel overwhelming. But you **don’t** have to learn **everything at once**. <br/>
...
It’s about **credibility** and **transparency**. <br/>

Reproducible science is **not** about being **perfect**. <br/>
